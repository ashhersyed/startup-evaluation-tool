...
export default class KhoslaScraper extends BaseScraper {
  name = 'khosla';

  private readonly BASE_URL = 'https://jobs.khoslaventures.com';
  private readonly JOBS_URL = 'https://jobs.khoslaventures.com/jobs';
  ...
  async scrape(): Promise<ScraperResult> {
    ...
    try {
      ...
      this.logError('Failed to scrape Khosla jobs', err);
    }
          ...
          try {
            ...
            if (title && company) {
              ...
              if (!companiesMap.has(company.toLowerCase())) {
                companiesMap.set(company.toLowerCase(), { name: company, investors: ['Khosla Ventures'] });
              }
            }
          } catch { /* skip */ }
        });
