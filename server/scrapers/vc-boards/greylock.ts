...
export default class GreylockScraper extends BaseScraper {
  name = 'greylock';

  private readonly BASE_URL = 'https://jobs.greylock.com';
  ...
  async scrape(): Promise<ScraperResult> {
    ...
    try {
      ...
      this.logError('Failed to scrape Greylock jobs', err);
    }
          ...
          try {
            ...
            if (title && company) {
              ...
              if (!companiesMap.has(company.toLowerCase())) {
                companiesMap.set(company.toLowerCase(), { name: company, investors: ['Greylock Partners'] });
              }
            }
          } catch { /* skip */ }
        });
