...
export default class ThriveScraper extends BaseScraper {
  name = 'thrive';

  private readonly BASE_URL = 'https://jobs.thrivecap.com';
  private readonly JOBS_URL = 'https://jobs.thrivecap.com/jobs';
  ...
  async scrape(): Promise<ScraperResult> {
    ...
    try {
      ...
      this.logError('Failed to scrape Thrive Capital jobs', err);
    }
          ...
          try {
            ...
            if (title && company) {
              ...
              if (!companiesMap.has(company.toLowerCase())) {
                companiesMap.set(company.toLowerCase(), { name: company, investors: ['Thrive Capital'] });
              }
            }
          } catch { /* skip */ }
        });
