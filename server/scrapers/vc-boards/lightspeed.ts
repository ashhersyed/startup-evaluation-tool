...
export default class LightspeedScraper extends BaseScraper {
  name = 'lightspeed';

  private readonly BASE_URL = 'https://jobs.lsvp.com';
  ...
  async scrape(): Promise<ScraperResult> {
    ...
    try {
      ...
      this.logError('Failed to scrape Lightspeed jobs', err);
    }
          ...
          try {
            ...
            if (title && company) {
              ...
              if (!companiesMap.has(company.toLowerCase())) {
                companiesMap.set(company.toLowerCase(), { name: company, investors: ['Lightspeed Venture Partners'] });
              }
            }
          } catch { /* skip */ }
        });
