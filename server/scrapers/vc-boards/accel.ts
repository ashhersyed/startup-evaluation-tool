...
export default class AccelScraper extends BaseScraper {
  name = 'accel';

  private readonly BASE_URL = 'https://jobs.accel.com';
  private readonly JOBS_URL = 'https://jobs.accel.com/jobs';
  ...
  async scrape(): Promise<ScraperResult> {
    ...
    try {
      ...
      this.logError('Failed to scrape Accel jobs', err);
    }
          ...
          try {
            ...
            if (title && company) {
              ...
              if (!companiesMap.has(company.toLowerCase())) {
                companiesMap.set(company.toLowerCase(), { name: company, investors: ['Accel'] });
              }
            }
          } catch { /* skip */ }
        });
