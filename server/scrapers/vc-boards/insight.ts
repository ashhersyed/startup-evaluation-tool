...
export default class InsightScraper extends BaseScraper {
  name = 'insight';

  private readonly BASE_URL = 'https://jobs.insightpartners.com';
  private readonly COMPANIES_URL = 'https://jobs.insightpartners.com/companies';
  ...
  async scrape(): Promise<ScraperResult> {
    ...
    try {
      ...
      this.logError('Failed to scrape Insight Partners jobs', err);
    }
          ...
          try {
            ...
            if (title && company) {
              ...
              if (!companiesMap.has(company.toLowerCase())) {
                companiesMap.set(company.toLowerCase(), { name: company, investors: ['Insight Partners'] });
              }
            }
          } catch { /* skip */ }
        });
