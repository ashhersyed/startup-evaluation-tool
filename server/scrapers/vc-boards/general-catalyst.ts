...
export default class GeneralCatalystScraper extends BaseScraper {
  name = 'general-catalyst';

  private readonly BASE_URL = 'https://jobs.generalcatalyst.com';
  private readonly JOBS_URL = 'https://jobs.generalcatalyst.com/jobs';
  ...
  async scrape(): Promise<ScraperResult> {
    ...
    try {
      ...
      this.logError('Failed to scrape General Catalyst jobs', err);
    }
          ...
          try {
            ...
            if (title && company) {
              ...
              if (!companiesMap.has(company.toLowerCase())) {
                companiesMap.set(company.toLowerCase(), { name: company, investors: ['General Catalyst'] });
              }
            }
          } catch { /* skip */ }
        });
